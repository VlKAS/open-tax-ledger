...
export function buildReviewModel(parsed, options = {}) {
  ...
  const faA3 = scheduleFa.filingRows.map(sanitizeScheduleFaA3Row);
  ...
  return {
    ...
    schedules: {
      ...
      fsi,
      tr,
      fa: fa.map(sanitizeScheduleFaEntity),
      faA3,
      holdings,
    },
    validations,
...
function sanitizeScheduleFaEntity(entity) {
  const { filingRows: _filingRows, ...entityRow } = entity;
  return entityRow;
}

function sanitizeScheduleFaA3Row(row) {
  return {
    filingEntityId: row.filingEntityId,
    filingRowId: row.filingRowId,
    acquisitionLotNumber: row.acquisitionLotNumber,
    ...
    initialValueStatus: row.initialValueStatus,
    grossAmountPaidOrCredited: row.grossAmountPaidOrCredited,
    grossAmountStatus: row.grossAmountStatus,
    ...
    saleRedemptionStatus: row.saleRedemptionStatus,
    saleEvents: (row.saleEvents ?? []).map((event) => ({
      date: event.date,
...
function normalizeTrade(entry, findings) {
  ...
  const dateTime = pick(row, ["Date/Time", "DateTime"]);
  const tradeDate = dateTime || pick(row, ["Date", "Trade Date", "TradeDate"]);
  const tradeTime = pick(row, ["Trade Time", "TradeTime", "Time"]);
  ...
  const price = numericField(pick(row, ["T. Price", "Trade Price", "TradePrice", "Price"]), {
    field: "price",
    section: "Trades",
    source: entry.source,
    findings,
  ...
  const basis = numericField(pick(row, ["Basis", "Cost Basis", "CostBasis"]), {
    allowBlank: true,
    blankValue: 0,
    field: "basis",
    section: "Trades",
  ...
  return {
    type: "trade",
    assetCategory,
    unsupported: assetCategory ? !STOCK_CATEGORIES.has(assetCategory) : false,
    symbol: pick(row, ["Symbol", "Description"]),
    date,
    timestamp: normalizeTradeTimestamp(dateTime, date, tradeTime),
    currency: pick(row, ["Currency", "Currency Primary"]).toUpperCase(),
    ...securityIdentifiers(row),
...
function normalizeCash(entry, type, findings) {
  ...
  const explicitSymbol = pick(row, ["Symbol", "Ticker"]);
  const derivedSymbol = symbolFromDescription(description);
  ...
  return {
    ...
    symbol: explicitSymbol || derivedSymbol,
    symbolEvidence: explicitSymbol
      ? "explicit"
      : derivedSymbol
        ? "description"
        : "missing",
...
function normalizeOpenPosition(entry, findings) {
  ...
  const costBasis = numericField(
    pick(row, ["Cost Basis", "CostBasis", "Cost Basis Money", "CostBasisMoney", "Basis"]),
    {
    allowBlank: true,
    blankValue: 0,
    field: "cost basis",
  ...
  const closePrice = numericField(pick(row, [
    "Close Price",
    "ClosePrice",
    "Mark Price",
    "MarkPrice",
  ...
  const value = numericField(pick(row, [
    "Value",
    "Market Value",
    "Current Value",
    "Position Value",
  ...
  return {
    ...
    symbol: pick(row, ["Symbol", "Description"]),
    currency: pick(row, ["Currency"]).toUpperCase(),
    snapshotDate: isoDate(pick(row, ["Report Date", "ReportDate"])),
    ...securityIdentifiers(row),
    quantity: quantity.value,
...
function parsePeriod(value) {
  ...
  const dates = isoDates.length > 0 ? isoDates : humanReadableDates(raw);
  return {
    start: dates[0] ?? "",
    end: dates[1] ?? dates[0] ?? "",
  };
...
function normalizeTradeTimestamp(dateTime, date, separateTime) {
  if (!date) return "";
  const source = separateTime || dateTime;
  const match = String(source ?? "").match(
    /(?:^|[T,\s])(\d{1,2}):(\d{2})(?::(\d{2}))?\s*(AM|PM)?\b/i,
  );
  if (!match) return "";
...
function securityIdentifiers(row) {
  return {
    conid: pick(row, ["Conid", "ConID", "Contract ID", "ContractID"]),
    isin: pick(row, ["ISIN"]),
    figi: pick(row, ["FIGI"]),
    cusip: pick(row, ["CUSIP"]),
    securityId: pick(row, ["Security ID", "SecurityID"]),
  };
}

function instrumentAuditKey(row) {
  const identifiers = [
    ["conid", row?.conid],
    ["isin", row?.isin],
    ["figi", row?.figi],
...
function humanReadableDates(value) {
  const monthNumbers = {
    JANUARY: 1,
    FEBRUARY: 2,
    MARCH: 3,
  ...
  for (const match of String(value ?? "").matchAll(pattern)) {
    const month = monthNumbers[match[1].toUpperCase()];
