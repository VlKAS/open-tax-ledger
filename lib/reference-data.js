...
export function enrichReviewWithReferenceData(review, options = {}) {
  ...
  const faA3 = enrichSecurityRows(schedules.faA3, companyLookup).map((row) =>
    attachScheduleFaA3Conversions(row, rateTable),
  );
...
function attachScheduleFaA3Conversions(row, rateTable) {
  const initial = attachValueDateConversion(
    row.currency,
    row.acquisitionDate,
    row.initialValue,
  ...
  const saleEvents = (row.saleEvents ?? []).map((event) => {
    const converted = attachRule115Conversion(
      {
        ...event,
    ...
    return {
      ...
      proceedsInr: converted.conversion.amountInr,
    };
  });
  const convertedSaleEvents = saleEvents.filter((event) =>
    Number.isFinite(event.conversion?.amountInr),
  );
  const saleRedemptionProceedsInr =
    saleEvents.length === 0
      ? row.saleRedemptionProceeds === 0
        ? 0
        : null
      : convertedSaleEvents.length === saleEvents.length
        ? Math.round(
            convertedSaleEvents.reduce(
              (total, event) => total + event.conversion.amountInr,
              0,
            ),
          )
        : null;
  const grossAmountPaidOrCredited = saleAggregateConversion(
    row,
    saleEvents,
    saleRedemptionProceedsInr,
  );

  return {
    ...row,
    valueConversions: { initial, peak, closing, grossAmountPaidOrCredited },
    saleEvents,
    initialValueInr: initial.amountInr,
    peakValueInr: peak.amountInr,
    closingValueInr: closing.amountInr,
    grossAmountPaidOrCreditedInr: saleRedemptionProceedsInr,
    saleRedemptionProceedsInr,
  };
}

function saleAggregateConversion(row, saleEvents, amountInr) {
  const completeStatuses = new Set(["matched", "not-required"]);
  const amountForeign =
    finiteNumber(row.saleRedemptionProceeds) ??
    saleEvents.reduce((total, event) => {
      const proceeds = finiteNumber(event.proceeds);
      return proceeds === null ? total : total + proceeds;
    }, 0);
  const base = {
    authority: "Rule 115",
    category: "gross-amount-paid-credited",
    eventDate: "",
    specifiedDate: "",
    dateRule:
      "Schedule FA A3 gross amount paid/credited uses the same per-sale converted proceeds as the sale/redemption total.",
    classificationReview: false,
    rate: null,
    sourceUrl: "",
    observations: [],
    observationDate: "",
    selection: "sale-events",
    daysPrior: null,
    currency: String(row.currency ?? "").trim().toUpperCase(),
    amountForeign,
    amountInr,
    exactDateOnly: saleEvents.every(
      (event) =>
        event.conversion?.status === "not-required" ||
        (event.conversion?.status === "matched" &&
          event.conversion?.selection === "exact"),
    ),
    saleEventConversions: saleEvents.map((event) => event.conversion),
  };

  if (saleEvents.length === 0) {
    return {
      ...base,
      status: amountInr === 0 ? "not-required" : "missing",
    };
  }

  const incomplete = saleEvents.find(
    (event) => !completeStatuses.has(event.conversion?.status),
  );
  return {
    ...base,
    status: incomplete?.conversion?.status ?? "matched",
  };
}
...
function summarizeFaConversions(schedules) {
  ...
  const a3Conversions = (schedules.faA3 ?? []).flatMap((row) => [
    ...Object.entries(row.valueConversions ?? {}).map(([field, conversion]) => ({
      field: `a3-${field}`,
      symbol: row.symbol,
      conversion,
    })),
    ...(row.saleEvents ?? []).map((event) => ({
      field: "a3-sale",
      symbol: row.symbol,
  ...
  const rows = [...entityConversions, ...a3Conversions, ...holdingConversions].filter(
    (row) => row.conversion,
  );
  const completeStatuses = new Set(["matched", "not-required"]);
...
function reconcileConversionValidations(
  ...
  if (
    [
      ...
      ...(schedules.faA3 ?? []).flatMap((row) => [
        ...Object.values(row.valueConversions ?? {}),
        ...(row.saleEvents ?? []).map((event) => event.conversion),
      ]),
    ].some(
      (conversion) =>
